sketch = """7-LJ.FF7J7-F-F--F7-.LL|7.LFJ--7F.J.L--FF-F77-F7|7F-F-777.JJ7|.J--F7JF-7-7-F-F-J.F-FF--7..|.LFJ77.77|F7FJ-77|F77FFL|7.|-7.|.7JFF--7|.|7-F--77
...
def find_first_step(g, s):
    if s[0] - 1 > 0 and g[s[0] - 1][s[1]] in ['|', '7', 'F']:
        return s[0] - 1, s[1]
    if s[1] + 1 < len(g[s[0]]) and g[s[0]][s[1] + 1] in ['-', 'J', '7']:
        return s[0], s[1] + 1
    if s[0] + 1 < len(g) and g[s[0] - 1][s[1]] in ['|', 'L', 'J']:
        return s[0] + 1, s[1]
    if s[1] - 1 > 0 and g[s[0]][s[1] - 1] in ['-', 'L', 'F']:
        return s[0], s[1] - 1


def find_next_steps(g, c):
    if g[c[0]][c[1]] == '|':
...
if __name__ == '__main__':
    graph = sketch.splitlines()
    start = find_start(graph)
    current = find_first_step(graph, start)
    last = start
    steps = 0
    points = [start]
    while True:
        steps += 1
        if graph[current[0]][current[1]] in ['L', 'J', '7', 'F', '|', '-']:
            points.append(current)
        nexts = find_next_steps(graph, current)
        for next in nexts:
            if next == last:
                continue
            last = current
    ...
    for y in range(len(graph)):
        ...
        for x in range(len(graph[y])):
            if (y, x) in points:
                polygon[y].append(graph[y][x])
            else:
                polygon[y].append('.')

    area = 0
    for y in range(1, len(graph) - 1):
        for x in range(1, len(graph[y]) - 1):
            if polygon[y][x] != '.':
                continue
            count = 0
            for i in range(0, x):
                ...
                if polygon[y][i] == '|':
                    count += 1
                    continue
                if polygon[y][i] in ['L', '7']:
                    count += 0.5
                    continue
                if polygon[y][i] in ['F', 'J']:
                    count -= 0.5
            if int(count) % 2 == 1:
                area += 1
