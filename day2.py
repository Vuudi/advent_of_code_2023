example = """Game 1: 1 green, 6 red, 4 blue; 2 blue, 6 green, 7 red; 3 red, 4 blue, 6 green; 3 green; 3 blue, 2 green, 1 red
...
if __name__ == '__main__':
    ...
    for line in example.splitlines():
        ...
        for cube_set in cube_sets:
            cu = cube_set.split(', ')
            for c in cu:
                if c.split(' ')[1] in cube_dict:
                    cube_dict[c.split(' ')[1]] = max(cube_dict[c.split(' ')[1]], int(c.split(' ')[0]))
                else:
                    cube_dict[c.split(' ')[1]] = int(c.split(' ')[0])
        power = 1
        for color in cube_dict:
            power *= cube_dict[color]
        sum += power
