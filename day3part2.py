example = """.....613...................................439............498.........................438......617....343.............942...................
...
def is_symbol(m, a, b):
    return -1 < a < len(m) and -1 < b < len(m[a]) and m[a][b] == '*'


if __name__ == '__main__':
    matrix = example.splitlines()
    gearMatrix = {}
    for y in range(len(matrix)):
        ...
        for x in range(len(matrix[y])):
            if matrix[y][x].isdigit():
                currentNumber += matrix[y][x]
                adjacentCoords = [(y - 1, x), (y + 1, x), (y, x - 1), (y, x + 1), (y - 1, x - 1), (y - 1, x + 1),
                                   (y + 1, x - 1), (y + 1, x + 1)]
                for i, j in adjacentCoords:
                    if is_symbol(matrix, i, j):
                        symbolFound = True
                        symbolCoords = (i, j)
            else:
                if currentNumber != '' and symbolFound:
                    if (symbolCoords[0], symbolCoords[1]) not in gearMatrix:
                        gearMatrix[(symbolCoords[0], symbolCoords[1])] = []
                    gearMatrix[(symbolCoords[0], symbolCoords[1])].append(int(currentNumber))
                currentNumber = ''
                symbolFound = False
        if currentNumber != '' and symbolFound:
            if (symbolCoords[0], symbolCoords[1]) not in gearMatrix:
    ...
    sum = 0
    for c in gearMatrix:
        if len(gearMatrix[c]) > 1:
            prod = 1
            for a in gearMatrix[c]:
                prod *= a
            sum += prod
